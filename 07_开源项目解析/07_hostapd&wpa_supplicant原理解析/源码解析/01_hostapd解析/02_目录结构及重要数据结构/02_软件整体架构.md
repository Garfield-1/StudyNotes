...
由于简中网络和`hostapd`官网上的大量资料，均是使用旧版本，这里分析时会结合`0.5.10`与`2.9`版本的`hostapd`共同分析
...
上图来自`hostapd`官网列出的一份`0.5.X`源码说明文档[hostapd-devel.pdf](https://hostap.epitest.fi/hostapd/hostapd-devel.pdf)，此版本时间约为`2008`年，这里**仅作一份参考**；
...
2. **关键功能**
    ...
    * 权限控制：通过`socket`目录权限和group配置，限制哪些用户/进程可以访问控制接口。
