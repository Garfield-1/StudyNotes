...
| `CONFIG_DRIVER_RADIUS_ACL`        | `CONFIG_DRIVER_RADIUS_ACL` 用于在hostapd中启用**`RADIUS ACL`（`Access Control List`）驱动接口** 的支持，适用于需要`RADIUS`认证但不需要完整`IEEE 802.11`管理功能的场景，特别是在`FreeBSD`系统中。它提供了灵活的用户认证和访问控制功能，但需要依赖外部`RADIUS`服务器 |
