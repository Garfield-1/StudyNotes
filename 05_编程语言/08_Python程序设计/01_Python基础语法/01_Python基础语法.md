...
| ------------- | ---------------------- | ---------------------- | ------------------------------- |
| 语法符号      | 方括号 `[]`            | 圆括号 `()`            | 花括号 `{键: 值}`               |
| 可变性        | 可变 (`Mutable`)       | 不可变 (`Immutable`)   | 可变 (`Mutable`)                |
| 有序性        | 有序 (按插入顺序)      | 有序 (按插入顺序)      | `Python 3.7+` 有序 (按插入顺序) |
| 元素/键唯一性 | 元素可重复             | 元素可重复             | 键 (`Key`) 必须唯一             |
| 访问方式      | 通过数字索引 (`Index`) | 通过数字索引 (`Index`) | 通过键 (`Key`)                  |
| 查找速度      | 较慢 (`O(n)`)          | 较慢 (`O(n)`)          | 极快 (`O(1)`)                   |
